...
func (h *handler) InsertArticle(ctx echo.Context) (err error) {
	...
	article, err := h.usecase.InsertArticle(ctx.Request().Context(), title, slug, htmlcontent, categoryid)
	if err != nil {
...
func (h *handler) UpdateArticle(ctx echo.Context) (err error) {
	...
	article, err := h.usecase.UpdateArticle(ctx.Request().Context(), id, title, slug, htmlcontent, categoryid)
	if err != nil {
		if err == sql.ErrNoRows {
